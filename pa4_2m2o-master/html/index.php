<?php
   ...
   session_save_path("/var/www/html/group54/admin/pa4/php/session");
   session_start();
...
/*--------------------------INDEX------------------------------------*/
   ...
   $klein->respond('GET', '/ymneig/pa4/', function ($request, $response, $service) use ($smarty) {
        session_save_path("/var/www/html/group54/admin/pa4/php/session");
	session_start();
...
/*-------------------------------- Get search----------------------------------*/
$klein->respond('GET', '/ymneig/pa4/search', function ($request, $response, $service) use ($smarty) {
...
/*--------------------------------Post search----------------------------------*/
$klein->respond('POST', '/ymneig/pa4/search', function ($request, $response, $service) use ($smarty) {
	...
	if($_POST['submit']=='Search')
	{
		require "server.php";
		$query = $_POST['query'];
		$myResults = queryIndex(6054,"localhost",$query);
		$urls = array();
		$captions = array();
		...
		{
			$id = $myResults[$i]["id"];
                	$queryFormat= "select url,caption from Photo where sequencenum='".$id."';";
	...
	if($_POST['clickImg']=='true')
	{
		require "server.php";
		$myResults = queryIndex(6054,"localhost",$_POST['caption']);
		$urls = array();
                $captions = array();
                for($i = 0;$i<sizeof($myResults);$i++)
                {
...
/*-----------------------------------login--------------------------------*/
$klein->respond('GET', '/ymneig/pa4/login', function ($request, $response, $service) use ($smarty) {
...
/*--------------------------------------login-POST------------------------*/
$klein->respond('POST', '/ymneig/pa4/login', function ($request, $response, $service) use ($smarty)
{
	session_save_path("/var/www/html/group54/admin/pa4/php/session");
	session_start();
	...
	{
		$response->redirect('/ymneig/pa4/')->send();			
	}
	else
	{
...
/*---------------------------------------log out -------------------------------*/
$klein->respond('GET', '/ymneig/pa4/logout', function ($request, $response, $service) use ($smarty) {
...
$klein->respond('GET', '/ymneig/pa4/user', function ($request, $response, $service) use ($smarty) {
...
/*--------------------------------------new user POST-------------------------*/
$klein->respond('POST', '/ymneig/pa4/user', function ($request, $response, $service) use ($smarty)
{
        session_save_path("/var/www/html/group54/admin/pa4/php/session");
        session_start();
...
$klein->respond('GET', '/ymneig/pa4/user/edit', function ($request, $response, $service) use ($smarty)
{
        session_save_path("/var/www/html/group54/admin/pa4/php/session");
        session_start();
...
/*-----------------------------user-edit POST----------------------------*/
$klein->respond('POST', '/ymneig/pa4/user/edit', function ($request, $response, $service) use ($smarty)
{
        session_save_path("/var/www/html/group54/admin/pa4/php/session");
        session_start();
...
$klein->respond('GET', '/ymneig/pa4/user/delete', function ($request, $response, $service) use ($smarty) {
        session_save_path("/var/www/html/group54/admin/pa4/php/session");
        session_start();
        $con=mysql_connect("127.0.0.1", "group54", "group54") or die('Could not connect: ' .mysql_error());
        mysql_select_db("group54pa2", $con);
...
/*---------------------------GET-ALBUMSEDIT-------------------------------*/
   ...
   $klein->respond('GET', '/ymneig/pa4/albums/edit\?[:username]?', function ($request, $response, $service) use ($smarty)
   {
	session_save_path("/var/www/html/group54/admin/pa4/php/session");
        session_start();
	$loggedin = 0;
        ...
        if(isset($username) AND $currentTime-$_SESSION['lastActivity']<=300)
	{
        ...
        {
                $response->redirect('/ymneig/pa4/logout')->send();
        }
 
  });

/*---------------------------------POST-ALBUMSEDIT----------------------------*/
   $klein->respond('POST', '/ymneig/pa4/albums/edit\?[:username]?', function ($request, $response, $service) use ($smarty)
   {
	session_save_path("/var/www/html/group54/admin/pa4/php/session");
        session_start();
	$username = $_SESSION['userName'];
        ...
        if(isset($username) AND $currentTime-$_SESSION['lastActivity']<=300)
        {
                ...
                else if($_POST['op']=='add'and $titleLength > 0 )
        	{
     		...
     		$response->redirect('/ymneig/pa4/albums/edit?username='.$_GET['username'])->send();
	}
...
$klein->respond('GET', '/ymneig/pa4/pic\?[:id]?', function ($request, $response, $service) use ($smarty)
 {  
...
/*-------------------------ALBUM----------------------------------*/
   ...
   $klein->respond('GET', '/ymneig/pa4/album\?[:id]?', function ($request, $response, $service) use ($smarty)
   {
...
/*---------------------------- GET--ALBUMEDIT-------------------------------*/
   ...
   $klein->respond('GET','/ymneig/pa4/album/edit\?[:id]?', function ($request, $response, $service) use ($smarty)
   {
	session_save_path("/var/www/html/group54/admin/pa4/php/session");
        session_start();
        $username = $_SESSION['userName'];
...
/*-------------------POST--ALBUMEDIT-----------------------------*/
   ...
   $klein->respond('POST', '/ymneig/pa4/album/edit\?[:id]?', function ($request, $response, $service) use ($smarty)
{
	...
	session_save_path("/var/www/html/group54/admin/pa4/php/session");
		...
		if($_POST['submit']=='Make change')
		{
	 	...
	 	$response->redirect('/ymneig/pa4/album/edit?id='.$_GET['id'])->send();
		mysql_close();
	 	$smarty->display('albumedit.tpl');
	}
	else 
